...
else if ( $mode == 'read' )
{
	...
	$username_from = get_racenet_nickname($privmsg['username_1']);
	$user_id_from = $privmsg['user_id_1'];
	$username_to = get_racenet_nickname($privmsg['username_2']);
	...
	//
	$template->assign_vars(array(
		'MESSAGE_TO' => get_racenet_nickname($username_to),
		'MESSAGE_FROM' => get_racenet_nickname($username_from),
...
else if ( $submit || $refresh || $mode != '' )
{
	...
	if ( $preview && !$error )
	{
		...
		$template->assign_vars(array(
			...
			'MESSAGE_FROM' => get_racenet_nickname($userdata['username']), 
...
if ( $row = $db->sql_fetchrow($result) )
{
	...
	do
	{
		...
		$template->assign_block_vars('listrow', array(
			'ROW_COLOR' => '#' . $row_color,
			'ROW_CLASS' => $row_class,
			'FROM' => get_racenet_nickname($msg_username),
			'SUBJECT' => $msg_subject,
			'DATE' => $msg_date,
