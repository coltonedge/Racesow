...
// 
function prepare_post(&$mode, &$post_data, &$bbcode_on, &$html_on, &$smilies_on, &$error_msg, &$username, &$bbcode_uid, &$subject, &$message, &$poll_title, &$poll_options, &$poll_length, $forum_id = NULL, $topic_id = NULL)
{
    global $board_config, $userdata, $lang, $phpEx, $phpbb_root_path;

    if( is_spam( $subject .' '. $message ) !== false ) {
    
        $msg = 'please don\'t spam and flame here!!!';
    
        if( $mode == 'reply' || $mode == 'editpost' ) $error_msg = $msg .'<br/><br/>'. sprintf($lang['Click_return_topic'], '<a href="' . append_sid("viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id") . '">', '</a>');
        else $error_msg = $msg.'<br/><br/>'. sprintf($lang['Click_return_forum'], '<a href="' . append_sid("viewforum.$phpEx?" . POST_FORUM_URL . "=$forum_id") . '">', '</a>');
        return 0x666;
    }
    ...
    if (!empty($message))
    {
        ...
        // zolex: shutup fucking phpBB and let html be html :P
...
function zolex_get_ad() {

    srand();
    
    $arr = array(
                                'I AM A HAPPY WARSOW PLAYER',
                                'WARSOW-RACENET ROXX',
                                'I AM A LITTLE SPAMMER :>'
                                
                                
                            );
    return $arr[ intval( rand(1, count($arr)-1) ) ];
}


//
// zolex checks for spam :>
//
function is_spam( $string )
{
        $badwords = array('liganet\.info','cardfile2\.asp','tinylink\.com','getcard\.php','card\.asp','cardView\.asp','cardViewer.php', 'adult video','free video','free sex','fghh.info','viagra','penis','ringtone','cialis','hardcore','porn','blowjob','valium','morphium','cannabis','heroin','cocain');
       if( preg_match( '@('. join( '|', $badwords ) .')@is', $string, $hit ) ) {
           return $hit[1];
    }
    return false;
}
...
function submit_post($mode, &$post_data, &$message, &$meta, &$forum_id, &$topic_id, &$post_id, &$poll_id, &$topic_type, &$bbcode_on, &$html_on, &$smilies_on, &$attach_sig, &$bbcode_uid, $post_username, $post_subject, $post_message, $poll_title, &$poll_options, &$poll_length)
{
	...
	// racenet
	if ($mode == 'newtopic' && $forum_id == 1) {
	
		ob_start();
		$err = ini_get('error_reporting');
		ini_set('error_reporting', E_NONE);
		require_once('/srv/apache2/warsow-race.net/www/library/SAM/php_sam.php');
		...
		$body = strip_tags($post_message);
		$xml = '<?xml version="1.0"?><news><title><![CDATA['. $post_subject .']]></title><body><![CDATA['. $body .']]></body></news>';
		
		$conn->Send('topic://news', (object)array('body' => $xml));
		$conn->Disconnect();
		...
		$body = strip_tags($post_message);
		$xml = '<?xml version="1.0"?><post><subject><![CDATA['. $post_subject .']]></subject><user><![CDATA['. $post_username .']]></user><body><![CDATA['. $body .']]></body></post>';
		
		$conn->Send('topic://forum', (object)array('body' => $xml));
		$conn->Disconnect();
