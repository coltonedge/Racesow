	...
	if ( $search_results != '' )
	{
		...
		for($i = 0; $i < count($searchset); $i++)
		{
			...
			if ( $show_results == 'posts' )
			{
				...
				$poster .= ( $searchset[$i]['user_id'] != ANONYMOUS ) ? get_racenet_nickname($searchset[$i]['username']) : ( ( $searchset[$i]['post_username'] != "" ) ? get_racenet_nickname($searchset[$i]['post_username']) : $lang['Guest'] );
			...
			else
			{
				...
				$topic_author .= ( $searchset[$i]['user_id'] != ANONYMOUS ) ? get_racenet_nickname($searchset[$i]['username']) : ( ( $searchset[$i]['post_username'] != '' ) ? get_racenet_nickname($searchset[$i]['post_username']) : $lang['Guest'] );
				...
				$last_post_author = ( $searchset[$i]['id2'] == ANONYMOUS ) ? ( ($searchset[$i]['post_username2'] != '' ) ? get_racenet_nickname($searchset[$i]['post_username2']) . ' ' : $lang['Guest'] . ' ' ) : '<a href="' . append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . '='  . $searchset[$i]['id2']) . '">' . get_racenet_nickname($searchset[$i]['user2']) . '</a>';
