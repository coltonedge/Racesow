...
function fix_schema($sql_layer)
{
	...
	$tables = array(
		...
		POSTS_TABLE => array(
			'post_edit_time' => TYPE_INT,
		),
		THEMES_TABLE => array(
			'fontsize1' => TYPE_INT,
			'fontsize2' => TYPE_INT,
			'fontsize3' => TYPE_INT,
			'img_size_poll' => TYPE_INT,
			'img_size_privmsg' => TYPE_INT,
		),
		TOPICS_WATCH_TABLE => array(
			'topic_id' => TYPE_INT,
			'user_id' => TYPE_INT,
		),
		USER_GROUP_TABLE => array(
			'user_pending' => TYPE_INT,
		),
		USERS_TABLE => array(
			'user_active' => TYPE_INT,
			'user_style' => TYPE_INT,
			'user_emailtime' => TYPE_INT,
			'user_viewemail' => TYPE_INT,
			'user_attachsig' => TYPE_INT,
			'user_rank' => TYPE_INT,
			'user_level' => TYPE_INT,
			'user_notify' => TYPE_INT,
		),
	);

	// process
	foreach ( $tables as $table_name => $fields_def )
	{
		foreach ( $fields_def as $field_name => $field_type )
		{
			$sql = 'UPDATE ' . $table_name . '
						SET ' . $field_name . ' = ' . ($field_type == TYPE_INT ? 0 : '\'\'') . '
			...
			{
				case 'mssql':
					$sql = 'ALTER TABLE [' . $table_name . '] WITH NOCHECK ADD CONSTRAINT [DF_' . $table_name . '_' . $field_name . '] NOT NULL DEFAULT (' . ($field_type == TYPE_INT ? 0 : '\'\'') . ') FOR [' . $field_name . ']';
					$db->sql_query($sql, false, __LINE__, __FILE__, false);
					break;

				case 'mysql':
					$sql = 'ALTER TABLE ' . $table_name . ' CHANGE ' . $field_name . ' ' . $field_name . ' NOT NULL DEFAULT \'' . ($field_type == TYPE_INT ? 0 : '') . '\'';
					$db->sql_query($sql, false, __LINE__, __FILE__, false);
					break;

				case 'postgres':
					$sql = 'ALTER TABLE ' . $table_name . ' ALTER COLUMN ' . $field_name . ' SET DEFAULT ' . ($field_type == TYPE_INT ? 0 : '\'\'');
					$db->sql_query($sql, false, __LINE__, __FILE__, false);
...
$lang['CH_code_name'] = '"The Halloween cruising of the Mary-Celeste revival" edition';
...
if ( step() == 'CH_dbstruct' )
{
	...
	// fix the phpBB structure
	fix_schema($sql_layer['SCHEMA']);
	...
	switch ( $parms['chpv'] )
	{
		...
		case '2.1.0f':
			$dbms_schema = 'schemas/' . $sql_layer['SCHEMA'] . '_2_1_0f.sql';
		...
		case '2.1.1':
			$dbms_schema = 'schemas/' . $sql_layer['SCHEMA'] . '_2_1_1RC5.sql';
			$wdata = @fread(@fopen($dbms_schema, 'r'), @filesize($dbms_schema));
			if ( $wdata )
			{
				$data .= "\n" . trim($wdata);
		...
		case '2.1.1RC6':
			$dbms_schema = 'schemas/' . $sql_layer['SCHEMA'] . '_2_1_1RC6.sql';
			$wdata = @fread(@fopen($dbms_schema, 'r'), @filesize($dbms_schema));
			if ( $wdata )
			{
				$data .= "\n" . trim($wdata);
		...
		case '2.1.4':
		case '2.1.4c':
			$dbms_schema = 'schemas/' . $sql_layer['SCHEMA'] . '_2_1_4.sql';
			$wdata = @fread(@fopen($dbms_schema, 'r'), @filesize($dbms_schema));
			if ( $wdata )
			{
				$data .= "\n" . trim($wdata);
