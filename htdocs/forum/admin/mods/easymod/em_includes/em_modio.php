 ...
 *
 *   $Id: em_modio.php,v 1.7 2005/10/31 17:57:15 markus_petrux Exp $
 *
 *
 ***************************************************************************/
...
////////// NOT WORRYING ABOUT READ LOCAL YET!!!
	...
	function modio_close( $create_command = false)
	{
		...
		if ( $this->write_method == 'server')
		{
			...
			if ($create_command)
			{
				chmod( $install_path . $this->path . trim($this->filename), 0777 ) ;
			}

			// normal files: written in the processed/ dir; add .txt on the end for security (not executable)
			else
			{
				chmod( $install_path . 'processed/' . $this->path . trim($this->filename) . '.txt', 0777 ) ;
			}
		}
