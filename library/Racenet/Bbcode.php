 ...
 * @package stringparser
 */
class Racenet_Bbcode extends StringParser {
    
    static $_instance = null;
    
    public static function getInstance($simple = false)
    {
        if (null === self::$_instance) {
            
            self::$_instance = new self($simple);
        }
    ...
    function __construct( $simple = false ) {
        ...
        $this->addCode( 'b', 'simple_replace', null, array ('start_tag' => '<b>', 'end_tag' => '</b>'), 'inline', array ('listitem', 'block', 'inline', 'link'), array () );
        $this->addCode( 'u', 'simple_replace', null, array ('start_tag' => '<u>', 'end_tag' => '</u>'), 'inline', array ('listitem', 'block', 'inline', 'link'), array () );
        $this->addCode( 'i', 'simple_replace', null, array ('start_tag' => '<i>', 'end_tag' => '</i>'), 'inline', array ('listitem', 'block', 'inline', 'link'), array () );
        $this->addCode( 'color', 'usecontent?', 'do_bbcode_color', array ('usecontent_param' => 'default'), 'inline', array ('block', 'inline', 'link', 'listitem'), array () );
        $this->addCode( 'size', 'usecontent?', 'do_bbcode_size', array ('usecontent_param' => 'default'), 'inline', array ('block', 'inline', 'link', 'listitem'), array () );
        $this->addCode( 'url', 'usecontent?', 'do_bbcode_url', array ('usecontent_param' => 'default'), 'link', array ('listitem', 'block', 'inline'), array ('link') );
        $this->addCode( 'img', 'usecontent', 'do_bbcode_img', array (), 'image', array ('listitem', 'block', 'inline', 'link'), array () );
        $this->addCode( 'code', 'usecontent', 'do_bbcode_code', array (), 'inline', array ('block', 'inline'), array () );
        $this->addCode( 'quote', 'callback_replace', 'do_bbcode_quote', array (), 'inline', array ('block', 'inline'), array () );
    ...
    */    
        $this->setGlobalCaseSensitive (false);
...
class StringParser_BBCode_Node_Element extends StringParser_Node {
    ...
    function &_findPrevAdjentTextNodeHelper () {
        ...
        if( is_object( $lastnode ) ) if (!$lastnode->hadCloseTag ()) {
...
function do_bbcode_code ($action, $attributes, $content, $params, $node_object) {
    ...
    return '<div><b>[[code]]:</b></div><div style="background-color: #EFEFEF; border: 1px solid #ADADAD; padding: 7px;">' . stripslashes( $content ) . '</div>';
}
...
function do_bbcode_quote ($action, $attributes, $content, $params, $node_object) {
    ...
    $title = "[[Quote]]";
    if(!empty($attributes['default'])) $title .= " [[by]] " . $attributes['default'];
    return '<div><b>' . $title . '</b></div><div style="background-color: #EFEFEF; border: 1px solid #ADADAD; padding: 5px;">' . preg_replace( "/(<br \/*>){2,}/msi", "<br />", $content ) . '</div>';
}
